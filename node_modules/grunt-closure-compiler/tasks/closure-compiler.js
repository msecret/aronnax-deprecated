module.exports = function(grunt) {
    ...
    exec(command, { maxBuffer: data.maxBuffer*1024 }, function(err, stdout, stderr) {
      ...
      if (reportFile.length) {
        if (data.options.debug) {
          ...
          min_info(min, function(err, report) {
            if (err) {
              grunt.warn(err);
              done(false);
            }

            var string = stderr + report;

            // Write compile report to a file.
            fs.writeFile(reportFile, string, function(err) {
              if (err) {
                grunt.warn(err);
                done(false);
              }
  ...
  function min_info(min, onComplete) {
    gzip(min, function(err, buffer) {
      ...
      var reportString = 'Compressed size: ' + String((gzipSize / 1024).toFixed(2)).green + ' kb gzipped (' + String(gzipSize).green + ' bytes).';
      grunt.log.writeln(reportString);

      onComplete.call(this, null, reportString);
    });
  }
